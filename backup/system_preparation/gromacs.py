...
def check_atom_order(pdb_file, prep_file):
    ...
    atm_ord1 = [
        line.split()[2] for line in pdb if not any(x in line for x in ["TER", "END"])
    ]
    atm_ord2 = [
        line.split()[1]
        for line in prep
...
def run_tleap(wd, lig, pdb_path, PARM_DIR):
    ...
    lig_lines = (
        f"loadamberparams {lig_params}.frcmod\n" f"loadamberprep {lig_params}.prep"
    )
    pdb_lines = f"struc = loadpdb {pdb_path}"
    out_lines = (
        f"saveamberparm struc {wd}/{lig}_amb.top {wd}/{lig}_amb.crd\n"
...
def combine_gro(prot_path, lig_path, out_path=None):
    ...
    both_gro = chain(
        protein_gro[0], line2, protein_gro[2:-1], ligand_gro[2:-1], protein_gro[-1]
    )
    # Write the combined file to a new or custom path
    op = (
        f"{'/'.join(lig_path.split('/')[:-1])}/"
...
def combine_top(prot_top, lig_top, lig_name=None, directory=None):
    ...
    lig_res_name = (
        lig_name
        if lig_name
        else [st for st in lig if "moleculetype" in st][0].split("\n")[2].split()[0]
    )
...
def run_prep(out_dir, sys, lig=None, dif_size=False, ngroup=None):
    ...
    if ngroup:
        ...
        new_line = (
            f'echo -e "name {ngroup} Protein_LIG \\n q" '
        ...
        with open(f"{out_dir}/prep.sh", "w") as f:
            f.writelines(lines)
    try:
        subprocess.run(["bash", "prep_apo.sh"], cwd=f"{out_dir}", check=True)
    except subprocess.CalledProcessError as error:
        print(
            "Error code:", error.returncode, ". Output:", error.output.decode("utf-8")
        )
...
def setup_minim(dd, sys, lig=None, REMOTE=None):
    ...
    scripts = "/home/rhys/phd_tools/simulation_files/" "submission_scripts/Local_Dirs"
    # TODO -> Copy Files
    try:
        subprocess.run(f"cp {scripts}/01-Min/* {dd}/01-Min/", shell=True, check=True)
    except subprocess.CalledProcessError as error:
        print(
            "Error code:", error.returncode, ". Output:", error.output.decode("utf-8")
        )
    ...
    files = [f"{name}.top", f"{name}.gro", "i.ndx", "*.itp"]
    for fn in files:
        # TODO -> Copy Files
        try:
            subprocess.run(f"cp {dd}/00-Prep/{fn} {dd}/01-Min/", check=True, shell=True)
        except subprocess.CalledProcessError as error:
            print(
                "Error code:",
                error.returncode,
    ...
    if REMOTE:
        try:
            subprocess.run(
                ["rsync", "-avzhPu", f"{dd}/01-Min", f"{REMOTE}/{sys}+{lig}/"],
                check=True,
            )
        except subprocess.CalledProcessError as error:
            print(
...
def make_readable_gro(gro_path, top_path, tpr=None, out_path=None, reconstruct=False):
    ...
    if reconstruct:
        ...
        try:
            subprocess.run(
                (
                    "echo Protein System | gmx_mpi trjconv "
                    "-f /tmp/tmp1.gro "
                    f"-s {tpr} "
                    "-o /tmp/tmp2.gro "
                    "-pbc cluster "
...
def assign_weights(pdb, ligand, out_pdb=None):
    ...
    for i in np.arange(len(lines)):
        ...
        if any(
            x in lines[i] for x in ["REMARK", "TITLE", "TER", "CRYST", "MODEL", "WAT"]
        ):
            lines[i] = "DELETORiOUS"
        else:
            l = lines[i].split()
