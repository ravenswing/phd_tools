...
def _run_cpptraj(directory, input_file):
    ...
    try:
        subprocess.run(
            ["cpptraj", "-i", f"{directory}/{input_file}"], shell=True, check=True
        )
...
def snapshot_pdbs(directory, trj_path, top_path, ref_str, snapshots):
    ...
    # TODO -> Make dir
    try:
        subprocess.run(f"mkdir -p {directory}/snapshots/", shell=True, check=True)
    except subprocess.CalledProcessError as error:
        print(
            "Error code:", error.returncode, ". Output:", error.output.decode("utf-8")
        )
    ...
    if isinstance(snapshots[0], int):
        ...
        for snl in snapshots:
            ...
            file1.append((f"trajin {trj_path} " f"{' '.join([str(i) for i in snl])}"))
            # Load reference structure (.top + .r)
            if isinstance(ref_str, list):
                file1.append(f"parm {ref_str[0]} [refparm]")
                file1.append(f"reference {ref_str[1]} parm [refparm]")
            ...
            file1.append(
                (f"trajout {directory}/snapshots/{stem}.pdb " "multi keepext chainid A")
            )
            file1.append("go")
            # Write all to cpptraj input file
...
def autoimage_file(top_file, crd_file):
    ...
    pt.write_traj(out_name, to_convert, options="keepext", overwrite=True)
