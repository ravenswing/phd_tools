...
def import_basins(csv: str):
    return {
        k: b.iloc[:, 2:].values.tolist() for k, b in pd.read_csv(csv).groupby("sys")
    }
...
def com_dist(traj, atom_groupA, atom_groupB):
    ...
    crdsA = pd.DataFrame(md.compute_center_of_mass(trajA), columns=["xa", "ya", "za"])
    crdsB = pd.DataFrame(md.compute_center_of_mass(trajB), columns=["xb", "yb", "zb"])
    # Merge DataFrames
    crds = pd.concat([crdsA, crdsB], axis=1)
    # Calculate 3D distance between the two sets of coords.
    crds["dist"] = list(
        map(calc_3D_dist, crds.xa, crds.ya, crds.za, crds.xb, crds.yb, crds.zb)
    )
...
if __name__ == "__main__":
    ...
    lig_sides = {
        ...
        "MK87-RHS": ["C19", "C20", "C21", "C22", "C23", "C24", "O2", "O3", "O4"],
        "PF739-LHS": ["C20", "C21", "C22", "C23", "O5"],
        "PF739-RHS": ["C14", "C15", "C16", "C17", "C18", "C19", "O2", "O3", "O4"],
        "SC4-LHS": ["C2", "C3", "C4", "C6", "C16", "C23", "O3"],
        "SC4-RHS": ["C1", "C5", "C7", "C18", "C12", "C22", "C14", "O1", "O2"],
    }
    for method in METHODS:
        for system in SYSTS:
            with open(f"{OUT_DIR}/{method}_{system}_dists.csv", "w") as f:
                f.write(
                    ("system,ligand,replica,door,total_frames," "lhs,rhs,similar\n")
                )
            ...
            for lig in LIGS:
                for rep in REPS:
                    ...
                    # Load the trajectory
                    traj = md.load(
                        f"{wd}/{system}+{lig}_{rep}_GISMO.xtc", top=f"{wd}/md_dry.pdb"
                    )
                    # Align the trajectory - to first frame
                    traj_aligned = traj.superpose(traj, 0)
                    # Load in the COLVAR file
                    clv = (
                        colvar(f"{wd}/{system}+{lig}_{rep}_GISMO.colvar")
                        .drop([1])
                    ...
                    for door in ["front", "back"]:
                        b = (
                            [0.8, 1.5, 0.0, 1.0]
                            if door == "front"
                            else [0.0, 1.0, 0.75, 1.8]
                        )
                        # Extract indices from COLVAR
                        indices = clv.loc[
                            (clv["pp.proj"].between(b[0], b[1]))
                            & (clv["pp.ext"].between(b[2], b[3]))
                        ].index.values
                        print(len(indices))
                        ...
                        portal_atoms = traj.topology.select(
                            f"(resid {' or resid '.join([str(x) for x in portal_res])}) and rescode != H"
                        )
                        ...
                        for side in ["LHS", "RHS"]:
                            ...
                            ligand_atoms = traj.topology.select(
                                f"residue {lig_residue} and (name {' or name '.join(ligand_names)} )"
                            )
