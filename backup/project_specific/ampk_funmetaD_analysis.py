...
SAVE_DIR = (
    "/home/rhys/Dropbox/RESEARCH/AA_RHYS/BB_AMPK/Fun-metaD_Results/Plots_wReplicas"
)
    ...
    if replicas:
        for rep in ["R" + str(x) for x in np.arange(3) + 1]:
            ...
            for lig in LIGS:
                j = 0
                for system in SYSTS:
                    data, labels = load.fes(
                        f"{DATA_DIR}/{system}+{lig}/06-MetaD/{rep}/{system}+{lig}_FES",
                        False,
                    )
                    cmap = graphics.two_cv_contour(
                        data, labels, cmax, ax[i, j], funnel_parms
                    )
            ...
            cbar = plt.colorbar(
                cmap, cax=cax, aspect=10, ticks=np.arange(0.0, cmax + 1, 2.0)
            )
        ...
        plt.subplots_adjust(top=0.94, right=0.915)
        funnel_parms = {"lw": 0.0, "uw": 4.5, "sc": 2.5, "b": 1.0, "f": 0.15, "h": 1.5}
        i = 0
        for lig in LIGS:
            j = 0
            for system in SYSTS:
                data, labels = load.fes(
                    f"{DATA_DIR}/{system}+{lig}/06-MetaD/{system}+{lig}_FES", False
                )
                cmap = graphics.two_cv_contour(
                    data, labels, cmax, ax[i, j], funnel_parms
                )
...
def fes_strideplot(wd, name, cmax=32, stride=50, to_use=[0, 1, 2]):
    ...
    for i in np.arange(len(to_use)):
        ...
        i += 1
    cax = plt.axes([0.93, 0.11, 0.01, 0.77])
    cbar = plt.colorbar(cmap, cax=cax, aspect=10, ticks=np.arange(0.0, cmax, 2.0))
    cbar.set_label("Free Energy / kcal/mol", fontsize=10)
    fig.savefig(f"{SAVE_DIR}/FES_wStride_{name}.png", dpi=300, bbox_inches="tight")
...
def new_strideplot(wd, name, stride=50, to_use=[0, 1, 2], basins=None):
    ...
    for i in np.arange(len(to_use)):
        ...
        if basins is not None:
            ...
            ax[i].add_patch(b1)
            b2 = plt.Rectangle(
                (basins["unbound"][0], basins["unbound"][2]),
    ...
    cbar = plt.colorbar(cmap, cax=cax, aspect=10, ticks=np.arange(0.0, cmax, 2.0))
    cbar.set_label("Free Energy / kcal/mol", fontsize=10)
    fig.savefig(f"{SAVE_DIR}/NEW_FES_wStride_{name}.png", dpi=300, bbox_inches="tight")
...
def fes_by_replica(basins=None):
    for rep in ["R" + str(x) for x in np.arange(3) + 1]:
        ...
        cbar = plt.colorbar(cmap, cax=cax, aspect=10, ticks=np.arange(0.0, cmax, 2.0))
        cbar.set_label("Free Energy / kcal/mol", fontsize=10)
        fig.savefig(f"{SAVE_DIR}/{rep}_FES_multi.png", dpi=300, bbox_inches="tight")
...
def fes_by_system(ligand_list, system_list, n_reps, basins=None):
    for lig in ligand_list:
        for system in system_list:
            ...
            cax = plt.axes([0.98, 0.11, 0.01, 0.77])
            cbar = plt.colorbar(
                cmap, cax=cax, aspect=10, ticks=np.arange(0.0, cmax, 2.0)
            )
            cbar.set_label("Free Energy / kcal/mol", fontsize=10)
            fig.savefig(
                f"{SAVE_DIR}/FES/{system}+{lig}_FES.png", dpi=450, bbox_inches="tight"
            )
    ...
    for system in SYSTS:
        for lig in LIGS:
            for rep in ["R" + str(x) for x in np.arange(2) + 1]:
                ...
                gismo_traj(
                    wd,
                    f"metad_{system}+{lig}_final.xtc",
                    f"{system}+{lig}_{rep}_GISMO.xtc",
                )
                gismo_colvar(wd, out_colvar=f"{system}+{lig}_{rep}_GISMO.colvar")
