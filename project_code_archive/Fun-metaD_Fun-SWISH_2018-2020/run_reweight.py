...
PARSER.add_argument("-refpath", type=str, default='./reference.pdb',
                    ...
                    '(default: %(default)s)')
PARSER.add_argument("-simtime", type=int, default=500,
                    help='Time in ns to analyse until' +
                    '(default: %(default)s)')

# optional arguments
PARSER.add_argument("-tpr", type=str, default='mdrun.tpr',
                    help='tpr file to use (default: %(default)s)', required=False)
...
PARSER.add_argument("-swreps", type=int, default=None,
                    help='Number of SWISH replicas. Acts as SWISH analysis boolean. (default: %(default)s)',
                    required=False)
PARSER.add_argument("-swdir", type=str, default='/media/arc/cem/UCB_PROJECT/SWISH/C-TERM/5akk/SCALE_5/',
    ...
    if mode=='SWISH':
        ...
        try:
            subprocess.call("echo \'#!/bin/bash/\' > ./sumhills.sh",
                            shell=True)
            subprocess.call("echo \'cd {}{w}/\' >> ./sumhills.sh".format(EXTRA_DIR,w=wd),
                            shell=True)
            ...
            for i in np.arange(reps):
                subprocess.call('echo \"plumed sum_hills \
                                ...
                                .format(rep=i, s=stem, p=PDB, f=FS, t=TIME),
                                shell=True)
                subprocess.call("echo \'mkdir -p FES/fes_oT/rep{}\' >> ./sumhills.sh".format(i),
                                shell=True)
                subprocess.call('echo \"plumed sum_hills \
                                --hills hills/HILLS.{rep} \
                                --kt 2.5 \
    ...
    for rmsd in rmsds:
        ...
        try:
            subprocess.call("{pp} reweight.py \
                    ...
                    -rewcol {rwc} \
                    -outfile {of} \
                    -v".format(pp=PY_PATH, w=wd, n=nfes, cCp=comb_col_path,
...
def calculate_delta_g(fes_file, fes_dim,
    ...
    if fes_dim == '1D':
        if cutoff is not None:
            ...
            fes_data['fes_val'] = fes_data['fes_val'].apply(lambda x:
                                                            exp(-x / kT))
            ...
            in_out = list(map(lambda x: -kT * log(x / total) if x > 0
                          else np.inf, in_out))
        ...
        if A is not None and B is not None:
            fes_data = pd.concat([df[df.cv1 != "#!"] for df in
                                 pd.read_csv(fes_file,
                                             delim_whitespace=True,
            ...
            basin_A = fes_data[(fes_data.cv1.between(A[0], A[2]))
                               & (fes_data.cv2.between(A[3], A[1]))].fes_val
            basin_B = fes_data[(fes_data.cv1.between(B[0], B[2]))
                               & (fes_data.cv2.between(B[3], B[1]))].fes_val
...
def SWISH_process_delta_g(data_file, mode=None):
    ...
    if mode == 'oT':
        ...
        for r in np.arange(SWISH_NREPS):
            ...
            dGs = [] 
            for fes_file in sorted(glob.glob('{}/{}/FES/fes_oT/rep{}/fes_*'.format(EXTRA_DIR, wd, r)),\
                        key=lambda name: int(name.split('_')[-1][:-4])): 
                basinA = bsn.loc[(bsn.pdb == PDB) & (bsn.funnel == FS)].A.values[0]
                basinB = bsn.loc[(bsn.pdb == PDB) & (bsn.funnel == FS)].B.values[0]
                delta_g = calculate_delta_g(fes_file, '2D', A=basinA, B=basinB) 
...
def reweight_og_variables(mode=None):
    ...
    if mode == 'SWISH':
        for r in np.arange(SWISH_NREPS):
            ...
            # run reweighting python script individually for each CV
            try:
                subprocess.call(" mkdir {}/{}/FES/rew/".format(EXTRA_DIR, wd),
                                shell=True)
                subprocess.call("{pp} reweight.py \
                        -bsf 10 \
                        ...
                        -rewcol 3 4 \
                        -outfile {of} \
                        -v".format(pp=PY_PATH, E=EXTRA_DIR, w=wd, n=nfes, Cp=col_path,
                                   of=outfile, rep=r),
                                shell=True)
                print("SUCCESS: Output Reweighted Free Energy - {}".format(outfile))
            except:
...
def demux(reps):
    ...
    try:
        subprocess.call("echo Protein_LIG | gmx trjcat \
    ...
    for rep in np.arange(reps):
        try:
            # WHOLE
            subprocess.call("echo Protein_LIG | gmx trjconv -s {w}/{p}_NVT_prod0.tpr \
                            -f {w}/{n}_{p}_prod_scale5_{t}ns.xtc \
                            -o {w}/{n}_{p}_prod_scale5_{t}ns_whole.xtc \
                            -pbc whole \
                            -n {w}/index_{p}.ndx"
                            .format(w=SWD, p=PDB, t=TIME, n=rep), shell=True)
            # CLUSTER
            subprocess.call("echo Protein_LIG Protein_LIG | gmx trjconv -s {w}/{p}_NVT_prod0.tpr \
        ...
        try:
            ...
            subprocess.call("echo Backbone Protein_LIG | gmx trjconv -s {c}{p}.eqPR2.Ct.ALIGN.pdb \
