...
def hills_plot(hills_data, pdb, funnel_side, save_dir):
    ...
    plt.savefig(
        "{}/{m}-{f}_Heights.png".format(save_dir, f=funnel_side, m=pdb),
        bbox_inches="tight",
        dpi=300,
    )
...
def convergence(fes_dir, ts_list, ax):
    ...
    if "proj" in fes_dir:
        ...
        ax.set_xlim([-1, 17])
        ax.set_xticks(np.linspace(0.0, 15, num=4))
    ax.set_ylim([0, 20.0])
    ax.grid(alpha=0.5)
...
def diffusion(colv_data, pdb, cv, bounds, ax, lin_col):
    ...
    if cv == "proj":
        ax.set_ylim([-3.0, 50.0])
        ax.set_yticks(np.arange(6) * 10)
        ax.axhline(y=30, xmax=500.0, c="k", alpha=0.5, lw=1.0, ls="--")
        ax.axhline(
            y=bounds[0], xmax=500.0, c="k", alpha=0.5, lw=1.0, ls="--", zorder=20
        )
        ax.fill_between(x, 0, bounds[0] + bounds[1], color="k", alpha=0.1)
    else:
        ax.set_ylim([-1, 17])
        ax.set_yticks(np.linspace(0.0, 15, num=4))
    ax.set_xlim([0, 500.0])
...
def SWISH_diffusion(pdb, _lambda, ax, data, lin_col, n, label_N, bounds, demux=False):
    ...
    ax.set_ylim([-3, 50.0])
    ax.set_yticks(np.arange(6) * 10)
    ax.axhline(y=30, xmax=MAX, c="k", alpha=0.5, lw=1.0, ls="--")
    ax.axhline(y=bounds[0], xmax=MAX, c="k", alpha=0.5, lw=1.0, ls="--", zorder=20)
    ax.fill_between(x, 0, bounds[0] + bounds[1], color="k", alpha=0.1)
