...
colours = [
    "#31859C",  # FS1 & BS1 = RIGHT HAND SIDE
    "#FFC000",  # Tunnel
...
def fes_highlight(pdb):
    ...
    for fes_type in fes_list:
        ...
        for fs in ["FS1", "FS2"]:
            ...
            print(plot_num)
            fes_data = load.fes_simple(
                "{}/{}-{}_{}.fes".format(directory, pdb, fs, fes_type), is_rew
            )
            axis_labels = (
                ["pp.proj", "pp.ext"] if not is_rew else ["RMSD OUT", "RMSD IN"]
            )
            cmap = gr.two_cv_contour(
                fes_data,
                pdb,
    ...
    labels = {
        "OLD": ["Raw Data", "pp.proj / $\mathrm{\AA}$"],
...
def conv_multiplot(pdb_list, mode, cv="proj"):
    ...
    if mode == "all":
        ...
        fig.legend(
            [str(x) + " ns" for x in lines] + ["Reweight"],
            loc="lower center",
            ncol=6,
            frameon=False,
        )
        fig.savefig(
            "Figures/CONV_paper_" + "_".join(pdb_list) + ".png",
            dpi=300,
            bbox_inches="tight",
        )
    else:
...
def dif_multiplot2(pdb_list, cv="proj", rx=False):
        ...
        for pdb in pdb_list:
            ...
            for CV in cv_list:
                ...
                for fs in ["FS1", "FS2"]:
                    ...
                    gr.diffusion(
                        data, pdb, CV, bounds, axes[n + c, f], colours[c2p[pdb]]
                    )
                    f += 1
                c += 1
            n += 2
    ...
    fig.savefig(
        "Figures/DIFF_paper_{}.png".format(
            "_".join(pdb_list) if not rx else "_".join(pdb_list) + "_RX"
        ),
        dpi=300,
        bbox_inches="tight",
    )
...
def highlight5am0(cbar_max):
    ...
    for t in t_list:
        ...
        for fs in ["FS1", "FS2"]:
            ...
            # print(plot_num)
            fes_data, axis_labels = load.fes(
                "{}/5am0_{}/fes/fes_{}.dat".format(directory, fs, t), False
            )
            cmap = gr.two_cv_contour(
                fes_data,
                "5am0",
...
def highlight5am0_dgdt():
    ...
    for i in [0, 1]:
        ...
        fig.text(
            0.32 + (i * 0.4), 0.01, "Simulation Time / ns", ha="center", fontsize=10
        )
        fig.text(
            0.32 + (i * 0.4),
            0.9,
...
def dgdt_plots(pdb_list):
    ...
    for j in A:
        y_data = [
            database[pdb_list[j]].FS1.to_numpy(),
    ...
    fig.savefig(
        "Figures/dG_oT_" + "_".join(pdb_list) + ".png", dpi=300, bbox_inches="tight"
    )
...
def ddGdt_plot(n_reps, pdb_clust, ts, highlight=None):
    ...
    if len(ts) == 1:
        ...
        ax.axvline(x=300, ymin=-5.0, ymax=12.0, color="#0d47a1", ls="--")
    exp_vals = pd.read_csv("dg_values.csv", sep=",")
    ...
    for T in ts:
        ...
        for r in over:
            csv = "../../SWISH/ANALYSIS/SWISH_dGoT/{}ns/SWISH_dGoT_rep{}.csv".format(
                T, r
            )
            ...
            max_ylen = int(T / 10)
            x = np.linspace(0.0, max_ylen * 10, max_ylen + 1)
            if first:
                ax.plot(
                    x,
                    data[r][0],
    ...
    ax.axhline(y=0.0, xmin=0.0, xmax=max(x), c="xkcd:green", ls="--")
    ax.axhline(y=2.0, xmin=0.0, xmax=max(x), c="xkcd:green", alpha=0.2)
    ax.axhspan(
        ymin=0.0, ymax=2.0, xmin=0.0, xmax=max(x), facecolor="xkcd:green", alpha=0.1
    )
    ax.axhline(y=3.5, xmin=0.0, xmax=max(x), color="xkcd:orange", alpha=0.2)
    ax.axhspan(
        ymin=0.0, ymax=3.5, xmin=0.0, xmax=max(x), facecolor="xkcd:orange", alpha=0.1
    )
    ax.legend()

    ax.set_xlim([-5.0, max(ts)])
    ...
    ax.set_ylabel(
        "$\Delta\Delta$G / kcal $\mathrm{mol^{-1}}$",
        va="center",
        rotation="vertical",
        fontsize=10,
    )
    ...
    fig.savefig(
        "Figures/ddG_v_T_{}_{}{}.png".format(
            pdb_clust[0], max(ts), "" if highlight is None else "_H"
        ),
        dpi=300,
        bbox_inches="tight",
    )
...
def SWISH_diff_multi(pdb_list, size, demux=False, rx=False, boxes=False):
    ...
    fig = plt.figure(
        figsize=(size[0] * 4, size[1] * 4.5) if size == [3, 2] else (12, 6.5)
    )
    ...
    for n in A:
        ...
        if not demux:
            colvar_path = "/media/rhys/storage/UCB_Project/SWISH/ANALYSIS/{t}ns/{p}_FS2/colvars/COLVAR.{l}".format(
                p=pdb, l=_lambda, t=T
            )
        else:
            colvar_path = "/media/rhys/storage/UCB_Project/SWISH/ANALYSIS/{t}ns/{p}_FS2/demux/colvars/{l}_{p}_{t}ns_demux.colvar".format(
                p=pdb, l=_lambda, t=T
            )
        ...
        gr.SWISH_diffusion(
            pdb, _lambda, axes[n], data, lin_col, n, label_N, bounds, demux
        )
    k = 0
    for j in np.arange(size[0]):
        for i in np.arange(size[1]):
            ...
            if size == [3, 2]:
                ...
                fig.text(
                    0.135 + (i * 0.42),
                    0.3 + (np.arange(size[0])[-j - 1] * 0.272),
                    pdb_list[k],
                    ha="left",
                    fontsize=20,
                    color=lin_col,
            ...
            if size == [2, 2]:
                fig.text(
                    0.13 + (i * 0.4),
                    0.43 + (np.arange(size[0])[-j - 1] * 0.395),
                    pdb_list[k],
                    ha="left",
                    fontsize=20,
                    color=lin_col,
    ...
    if rx:
        s += "_RX"
    fig.savefig(
        "Figures/COMET_diffMULTI_{}.png".format(s), dpi=300, bbox_inches="tight"
    )
    plt.close(fig)
...
def SWISH_energies(pdb, n_res):
    ...
    fig.savefig(
        "Figures/SWISH_energies_{}.png".format(pdb), dpi=300, bbox_inches="tight"
    )
...
def SWISH_exchange(pdb, n_res):
    ...
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    index_file = (
        "/media/arc/cem/UCB_PROJECT/SWISH/C-TERM/{}/SCALE_5/replica_index.xvg".format(
            pdb
        )
    )
    df = pd.read_csv(
        ...
        sep="\s+",
        header=None,
        names=["time", "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7"],
    )
...
def SWISH_fes(pdb, n_res, time, cbar_max):
    ...
    axes = axes.ravel()
    directory = "/media/arc/cem/UCB_PROJECT/SWISH/ANALYSIS/{t}ns/{p}_FS2/FES/".format(
        t=time, p=pdb
    )
    ...
    for REP in np.arange(n_res):
        ...
        # print(plot_num)
        fes_data = load.fes_simple(
            "{d}{n}_{p}_FS2_{t}ns.fes".format(d=directory, p=pdb, n=REP, t=time), False
        )
        cmap = gr.two_cv_contour(
            fes_data,
            pdb,
    ...
    fig.savefig(
        "Figures/SWISH_FES_{}_{}ns.png".format(pdb, time), dpi=300, bbox_inches="tight"
    )
...
def performance_bar():
    ...
    bar1 = ax.barh(
        [y * scale for y in pos[0]],
        [vals[i] for i in pos[0]],
        width,
        label="Fun-metaD",
        color="#5a7d9a",
    )
    bar2 = ax.barh(
        [y * scale for y in pos[1]],
        [vals[i] for i in pos[1]],
        width,
        label="Fun-SWISH",
        color="#0a888a",
    )
    bar3 = ax.barh(
        [y * scale for y in pos[2]],
        [vals[i] for i in pos[2]],
        width,
        label="COMetPath",
        color="#1e488f",
    )
    ax.axhline(y=2.5 * scale, xmin=0.0, xmax=max(vals) + 10, c="xkcd:charcoal", ls="--")
...
def performance_bar2():
    ...
    vals = [
        np.array([35.56, 32.9, 25.1, 38.5, 54.0, 42.0]),
    ...
    BARS = [None] * 6
    BARS[5] = ax.barh(
        ind, vals[5] - vals[4], label="6 Nodes", alpha=0.25, left=vals[4], zorder=1
    )
    BARS[4] = ax.barh(
        ind, vals[4] - vals[3], label="5 Nodes", alpha=0.40, left=vals[3], zorder=2
    )
    BARS[3] = ax.barh(
        ind, vals[3] - vals[2], label="4 Nodes", alpha=0.55, left=vals[2], zorder=3
    )
    BARS[2] = ax.barh(
        ind, vals[2] - vals[1], label="3 Nodes", alpha=0.70, left=vals[1], zorder=4
    )
    BARS[1] = ax.barh(
        ind, vals[1] - vals[0], label="2 Nodes", alpha=0.85, left=vals[0], zorder=5
    )
    ...
    ax.axhline(y=2.5, xmin=0.0, xmax=85, c="xkcd:charcoal", ls="--")
