...
# make local directory
    try:
        subprocess.call(
            'mkdir -p {}{}'.format(srcdir,mol),
            shell=True)
    except:
        print( 'Unable to make directory: {}'.format(mol))
    try:
        subprocess.call(
            'cp Scripts/build.tleap {}{}/'.format(srcdir,mol), # LOCATION OF GLOBAL TLEAP FILES
            shell=True)
        subprocess.call(
            'cp Scripts/solv.tleap {}{}/'.format(srcdir,mol),
            shell=True)
    except:
...
# Solvate to get volume of box...
    sub = subprocess.Popen( 'tleap -f  {}/{}/solv.tleap'.format(srcdir,mol), 
...
# Utilise TLEaP
    sub = subprocess.Popen( 'tleap -f  {}/{}/build.tleap'.format(srcdir,mol), 
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE, 
        shell=True )
    out, errors = sub.communicate()
...
# Amber -> Gromace Conversion
    traj = pmd.load_file('{}{m}/{m}.prmtop'.format(srcdir,m=mol), '{}{m}/{m}.rst7'.format(srcdir,m=mol))
    traj.save('{}{m}/{m}.gro'.format(srcdir,m=mol))
    traj.save('{}{m}/{m}.top'.format(srcdir,m=mol))
    print( 'Amber -> Gromacs Conversion Complete\n \n Setup Completed for {}\n'.format(mol))
