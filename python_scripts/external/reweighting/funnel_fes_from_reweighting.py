...
# preliminary modification of the original reweighting script by Michele Invernizzi
# https://github.com/invemichele/opes/blob/master/postprocessing/FES_from_Reweighting.py
# so that the deltaF that it estimates includes the funnel correction
# the extra arguments to be used are rfunnel, uat, bat
...
parser.add_argument(
    ...
    type=float,
    required=True,
    help="the radius of the funnel, it MUST be in nm, the units of the CV z",
)
parser.add_argument(
    "--uat",
    dest="uat",
    type=float,
    required=True,
    help="the value of the z CV above which the ligand is unbound",
)
parser.add_argument(
    "--bat",
    dest="bat",
    type=float,
    required=True,
    help="the value of the z CV below which the ligand is bound",
)
...
if args.rfunnel is not None:
    calc_funnel = True
    pref = np.pi * (1 / 1.66) * args.rfunnel * args.rfunnel
    # pref=(np.pi*(1/1.66)*args.rfunnel*args.rfunnel*0.006)
    prefactorfunnel = np.log(
        pref
    )  # metti pure kT, così che è la correzione che si somma
    # print('using prefactor %g\n'%(prefactorfunnel))
    uat = args.uat
    bat = args.bat
...
while line[0] == "#!":
    ...
    if line[2] == "min_" + name_cv_x:
        ...
        if line[2] != "max_" + name_cv_x:
            sys.exit(
                error % ("min_%s was found, but not max_%s !" % (name_cv_x, name_cv_x))
            )
        if line[3] == "pi":
            grid_max_x = np.pi
        else:
    ...
    if dim2 and line[2] == "min_" + name_cv_y:
        ...
        if line[2] != "max_" + name_cv_y:
            sys.exit(
                error % ("min_%s was found, but not max_%s !" % (name_cv_y, name_cv_y))
            )
        if line[3] == "pi":
            grid_max_y = np.pi
        else:
...
def printFES(outfilename):
    ...
    if calc_funnel:
        if not dim2:
            # shift1=np.amin(fes)
            shift2 = np.average(fes[grid_cv_x > uat])
            deltax = (grid_max_x - grid_min_x) / (grid_bin_x - 1)
            extrafactor = np.log(deltax)
            fes2 = fes - shift2
            deltafunnel = -(
                -kbt * np.logaddexp.reduce(-1 / kbt * fes2[grid_cv_x < bat])
                - kbt * (prefactorfunnel + extrafactor)
            )
