...
parser = argparse.ArgumentParser(
    description="get the running FES estimate used by OPES, 1D only"
)
parser.add_argument(
    "--kernels",
    "-f",
...
parser.add_argument(
    ...
    type=float,
    required=True,
    help="the temperature in energy units",
)
parser.add_argument(
    "--angle",
    dest="angle",
    action="store_true",
    default=False,
    help="the cv is an angle in the range [-pi,pi]",
)
parser.add_argument(
    "--min",
    dest="grid_min",
...
parser.add_argument(
    ...
    type=float,
    required=False,
    help="upper bound for the grid",
)
parser.add_argument(
    "--bin", dest="grid_bin", type=int, default=100, help="number of bins for the grid"
)
parser.add_argument(
    "--stride",
    dest="stride",
    type=int,
    default=0,
    help="how often to print running fes",
)
parser.add_argument(
    "--mintozero",
    dest="mintozero",
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="skip derivative to run faster",
)
parser.add_argument(
    "--faster",
    dest="faster",
    action="store_true",
    default=False,
    help="run faster, ingoring Zed and the epsilon cutoff",
)
parser.add_argument(
    "--outfile",
    "-o",
...
print(
    " +++ WARNING: this bias reconstruction is approximate. Use FES_from_State.py for an exact result +++"
)
...
if line.split()[-1] == "OPES_METAD_kernels":
    ...
    sys.exit(
        "This script works only with OPES_METAD_kernels and OPES_METAD_EXPLORE_kernels"
    )
...
data = pd.read_table(
    filename, dtype=float, sep="\s+", comment="#", header=None, usecols=[1, 2, 3]
)
...
if print_stride <= len(center):
    ...
    if len(file_ext) > 1:
        file_ext = "." + file_ext
    current_fes_running = (
        "fes_running/" + outfile[: -len(file_ext)] + ".t-%d" + file_ext
    )
...
def get_merge_candidate(c, self):
    ...
    for j in range(len(z_center)):
        if j == self:
            continue
        dist = (
            abs(c - z_center[j]) / z_sigma[j]
        )  # merging through periodicity is not implemented in the code
        if dist < min_dist:
            min_j = j
    return min_j
...
def merge(j, m_height, m_center, m_sigma):
    ...
    s2 = (
        z_height[j] * (z_sigma[j] ** 2 + z_center[j] ** 2)
        + m_height * (m_sigma**2 + m_center**2)
    ) / h - c**2
    z_height[j] = h
    z_center[j] = c
...
def build_fes(c, s, h):
    ...
    for x in range(len(cv_grid)):
        ...
        if calc_der:
            der_prob[x] = np.sum(
                -dist / s * h * np.maximum(np.exp(-0.5 * dist**2) - val_at_cutoff, 0)
            )
    if not faster:
        Zed = 0
        for j in range(nker):
    ...
    return 0 - kbt * np.log(
        prob / norm
    ), 0 - kbt / prob * der_prob  # adding zero avoids printing -0
...
for i in range(1, len(center)):
    ...
    if (i + 1) % print_stride == 0:
        ...
        np.savetxt(
            current_fes_running % ((i + 1) * pace_to_time),
            np.c_[cv_grid, fes, der_fes],
            header=head,
            comments="",
            fmt="%14.9f",
        )
...
fes, der_fes = build_fes(z_center, z_sigma, z_height)
np.savetxt(
    outfile, np.c_[cv_grid, fes, der_fes], header=head, comments="", fmt="%14.9f"
)
