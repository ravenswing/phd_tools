...
for i in range(ncv):
    ...
    if line.split()[3 + ncv + i] != "sigma_" + cvname[i]:
        sys.exit(
            error
            % (
                ' expected "sigma_%s" instead of "%s"'
                % (cvname[i], line.split()[4 + i])
            )
        )
    ...
    if cvname[i].find(".") != -1:
        ...
        if (suffix[i] == ".x" or suffix[i] == ".y" or suffix[i] == ".z") and periodic[
            ...
            sys.exit(
                error
                % (
                    ' %s: you must modify the KERNELS file and remove any "." from CVs names'
                    % cvname[i]
                )
            )
...
plumed_input += (
    "COMMITTOR ARG=d BASIN_LL1=-1 BASIN_UL1=1\n"  # this will kill the driver
)
for i in range(ncv):
    if suffix[i] == "":
        plumed_input += (
