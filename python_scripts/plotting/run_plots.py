...
def fes_highlight(pdb):
    ...
    for fes_type in fes_list:
        ...
        n += 1
    plt.subplots_adjust(left=0.15)
    labels = {'OLD':['Raw Data', 'pp.proj / nm'],
              'REW':['Reweighted', axis_labels[0]+' / nm'],
             }
    # Y LABELS
    for i in [0, 1]:
        fig.text(0.04, 0.3+(i*0.38), 'FS'+str(i+1), ha='center', fontsize=14)
        fig.text(0.08, 0.31+(i*0.38), 'pp.ext / nm', va='center', rotation='vertical', fontsize=10)
...
def conv_multiplot(pdb_list, mode, cv='proj'):
    ...
    if mode == 'all':
        ...
        for i in A:
            ...
            fig.text(0.1, 0.16+(A[i]*0.132), '$\Delta$G / kcal/mol', va='center', rotation='vertical', fontsize=10)
        # X LABELS
        for i in np.arange(len(cv_list)):
            fig.text(0.33+(i*0.38), 0.92, 'pp.'+cv_list[i], ha='center', fontsize=14)
            for j in [0, 1]:
                n = 2*i+j
                fig.text(0.24+(n*0.19), 0.065, 'pp.'+cv_list[i]+' / nm', ha='center', fontsize=10)
...
def dif_multiplot(pdb_list):
    ...
    for i in A:
        ...
        fig.text(0.1, 0.225+(A[i]*0.275), '$\Delta$G / kcal/mol', va='center', rotation='vertical', fontsize=10)
...
def dif_multiplot2(pdb_list):
    ...
    for i in A:
        ...
        fig.text(0.1, 0.3+(A[-i-1]*0.265), 'pp.'+cv_list[0]+' / nm', va='center', rotation='vertical', fontsize=10)
        fig.text(0.1, 0.17+(A[-i-1]*0.265), 'pp.'+cv_list[1]+' / nm', va='center', rotation='vertical', fontsize=10)
...
pdb_16 = [['5am1', '5alg','5alp', '5alh'],['5ai5', '5aly', '5akh', '5am0'],['5alx', '5aia', '5alo', '5alt'], ['5akg','5akk', '5ai0', '5ak6']]
#for CV in ['proj','ext']:
    #for pdb_list in pdb:
