...
def processHelicity(traj, gro, frames):
    print("{:8}{m}".format("INFO:", m="Processing Helicity"))
    ...
    for chunk in md.iterload(xtc, 100, top=topol):
        ...
        for i in range(npep):
            peptide_chunk = chunk.atom_slice(
                chunk.top.select("resid {} to {}".format(i * nres, (i + 1) * nres - 1))
            )
            ...
            for j in np.arange(nres):
                df["pep" + str(i + 1), "res" + str(j + 1)] = pd.Series(
                    dssp[:, j]
                ).fillna(0)
...
def run(processing, mol):
    ...
    plt.yticks(fontweight="medium", fontsize=font_sizes[1])
    fig2.savefig(
        FIGDIR + uncapped_name + "_COMBIND.png",
        bbox_inches="tight",
        transparent=True,
        dpi=300,
    )
...
def analyse_trajectories():
    ...
    for hyd in hydrophobics:
        for ornt in orientations:
            traj = SRCDIR + "EK-{h}H/{o}/EK-{h}H_h2o/05-MD/EK-{h}H_{o}_cut.xtc".format(
                h=hyd, o=ornt
            )
            gro = (
                SRCDIR
                + "EK-{h}H/{o}/EK-{h}H_h2o/05-MD/EK-{h}H_h2o_protein.gro".format(
                    h=hyd, o=ornt
                )
            )
