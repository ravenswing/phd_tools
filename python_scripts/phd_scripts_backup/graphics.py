...
def hills_plot(hills_data, pdb, funnel_side, save_dir):
    ...
    plt.savefig(
        f"{save_dir}/{pdb}-{funnel_side}_Heights.png", bbox_inches="tight", dpi=300
    )
...
def single_diffusion_plots(colvar_data, pdb, funnel_side, num_cvs, save_dir):
    ...
    if num_cvs == 2:
        ...
        plt.ylim(-0.2, 5.0)
        plt.title(
            "{m}  |  {}  |  Projection on Z - pp.proj ".format(funnel_side, m=pdb)
        )
        plt.savefig(
            "{}/{m}-{f}_proj.png".format(save_dir, f=funnel_side, m=pdb),
            bbox_inches="tight",
            dpi=300,
        )
        plt.figure()
        ...
        plt.ylim(-0.1, 2.1)
        plt.title("{m}  |  {}  |  Distance from Z - pp.ext".format(funnel_side, m=pdb))
        plt.savefig(
            "{}/{m}-{f}_ext.png".format(save_dir, f=funnel_side, m=pdb),
            bbox_inches="tight",
            dpi=300,
        )
...
def two_cv_contour(
    fes, axes, in_vmax, ax, fp=None, pdb=None, funnel_side=None, name="fun-metad"
):
...
def diffusion(colv_data, cv, ax, lin_col):
    ...
    ax.plot(x, y2, c="k", alpha=1.0, lw=1.0)
    if cv == "proj":
        ax.set_ylim([-0.3, 5.0])
        ax.set_yticks(np.arange(6))
        ax.axhline(y=3.0, xmax=500.0, c="k", alpha=0.5, lw=1.0, ls="--")
        ax.axhline(y=0.7, xmax=500.0, c="k", alpha=0.5, lw=1.0, ls="--")
    else:
        ax.set_ylim([-0.1, 1.7])
        ax.set_yticks(np.linspace(0.0, 1.5, num=4))
    ax.set_xlim([0, 500.0])
    ax.set_xticks(np.linspace(0.0, 500.0, num=6))
    ax.grid(alpha=0.3)
...
def xvg_line(xvg_data, ax, col, line="solid", rollavg=True, label=None):
    ...
    if label:
        ...
        ax.plot(x, y2, c=col, alpha=1.0, lw=1.0, ls=line)
    ax.grid(alpha=0.3)
