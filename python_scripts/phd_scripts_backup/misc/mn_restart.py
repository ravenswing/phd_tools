...
try:
    subprocess.call(
        'rsync -avzhPe ssh --exclude "*.trr" {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/ Hydrophobic/{m}/06-RunMD/'.format(
            mn, m=mol
        ),
        shell=True,
    )
...
ls = subprocess.Popen(
    ["ssh", "{}".format(mn), "ls /gpfs/scratch/pr1eeq00/pr1eeq09/{}/".format(mol)],
...
nc = "gerun $GMX  mdrun -v -s mdrun.tpr -deffnm {} -tunepme -pin on -cpi {}.cpt  -append -maxh 23.5\n".format(
    newline, sorted(select, reverse=True)[0]
)
...
try:
    subprocess.call(
        "scp Scripts/Grace/mdrst.sh {}:~//gpfs/scratch/pr1eeq00/pr1eeq09/{}/".format(
            mn, mol
        ),
        shell=True,
    )
