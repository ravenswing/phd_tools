...
ibdir = 'ucb_run/Simulations/'
srcdir = 'ucb_run/Simulations/'
mols = ['5ai5']
...
for mol in mols:
    ...
    sub.stdin.write('pwd\n')
    sub.stdin.write('echo \'#!/bin/bash\n#$ -cwd -V\n#$ -q all.q\n#$ -pe openmpi 1\n\' >> select.sh\n')
    sub.stdin.write('echo \'export LD_LIBRARY_PATH=/usr/local/plumed-2.3.0/lib:$LD_LIBRARY_PATH\n\' >> select.sh\n')
    sub.stdin.write('echo \'export GMX=/usr/local/gromacs/5.1.4-plumed-2.3.0/bin/gmx_mpi\n\' >> select.sh\n')
    sub.stdin.write('echo \'echo 0 | $GMX trjconv -s md.tpr -f md.trr -o {m}_1barframe.gro -b {t} -e {t}\' >> select.sh\n'.format(m=mol, t=ts) )
