...
CVs = {
    "aRMSD": "a1: ALPHARMSD RESIDUES={}-{} TYPE=DRMSD LESS_THAN={{RATIONAL R_0=0.08 NN=8 MM=12}}".format(
        res[0], res[1]
    ),
}
...
for mol in mols:
    try:
        subprocess.call(
            "scp Scripts/driver.sh {}:{}{m}/".format(svr, svrdir, m=mol), shell=True
        )
        subprocess.call(
            "scp {}{m}/05-MD/{m}_protein.pdb {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/".format(
                srcdir, svr, m=mol
            ),
            shell=True,
        )
    ...
    try:
        ...
        sub.stdin.write(
            "echo 'MOLINFO STRUCTURE={m}_protein.pdb\n' >> plumed.dat\n".format(m=mol)
        )
        for cv in CVs:
            if cv not in use:
                continue
    ...
    try:
        subprocess.call(
            'rsync -rvzhPe ssh --exclude "*.trr" --ignore-existing {}:{}{m}/ {}{m}/06-RunMD/'.format(
                svr, svrdir, srcdir, m=mol
            ),
            shell=True,
        )
    except:
