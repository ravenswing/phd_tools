...
for mol in mols:
    ...
    try:
        subprocess.call(
            "ssh {} mkdir -p {}:~/{}{}".format(svr, svrdir, mol), shell=True
        )
    except:
    ...
    if ext:
        ...
        try:
            subprocess.call(
                "scp {}{m}/05-MD/{m}_1barframe.gro {}:~/{}{m}/".format(
                    srcdir, svr, svrdir, m=mol
                ),
                shell=True,
            )
            subprocess.call(
                "scp {}{m}/05-MD/{m}.top {}:~/{}{m}/".format(
                    srcdir, svr, svrdir, m=mol
                ),
                shell=True,
            )
            subprocess.call(
                "scp -r {}{m}/05-MD/{} {}:~/{}{m}/".format(
                    srcdir, watermodel[0], svr, svrdir, m=mol
                ),
                shell=True,
            )
            subprocess.call(
                "scp {}{m}/05-MD/min.tpr {}:~/{}{m}/".format(
                    srcdir, svr, svrdir, m=mol
                ),
                shell=True,
            )
            subprocess.call(
                "scp {}{m}/00-Prep/{m}_initial.pdb {}:~/{}{m}/".format(
                    srcdir, svr, svrdir, m=mol
                ),
                shell=True,
            )
            subprocess.call(
                "scp -r {}{m}/05B-SWISH/* {}:~/{}{m}/".format(
                    srcdir, svr, svrdir, m=mol
                ),
                shell=True,
            )
    ...
    try:
        subprocess.call(
            "scp Scripts/TH/* {}:~/{}{}/".format(svr, svrdir, mol), shell=True
        )
    except:
