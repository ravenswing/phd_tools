...
for mol in mols:
    ...
    try:
        subprocess.call(
            "scp {}{m}/{m}_initial.pdb {}:~/{}{m}/00-Prep/{m}_initial.pdb".format(
        ...
        if ligand:
            subprocess.call(
                "scp {}{m}/ligand*.itp {}:~/{}{m}/00-Prep/".format(
                    srcdir, ib, ibdir, m=mol
                ),
                shell=True,
            )
        if ligand:
            subprocess.call(
                "scp {}{m}/FRG_GMX.gro {}:~/{}{m}/00-Prep/".format(
                    srcdir, ib, ibdir, m=mol
                ),
                shell=True,
            )
        for f in folders:
            subprocess.call(
                "scp -r water_models/{} {}:~/{}{m}/{}".format(
                    watermodel[0], ib, ibdir, f, m=mol
                ),
                shell=True,
            )
    except:
