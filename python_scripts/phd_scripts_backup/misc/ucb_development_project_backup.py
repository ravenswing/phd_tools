...
trajectories = dict()
for t in traj_list:
    trajectories[t]=md.load_dcd('{}/{}/traj.dcd'.format(srcdir,t),\
                                top='{}/{}/input.pdb'.format(srcdir,t))
print('Successfully loaded : {}'.format(trajectories.keys()))

# align the trajectories to the reference
for t in traj_list[1:]:
    try:
        trajectories[t].superpose(trajectories[traj_list[0]],frame=0, \
            atom_indices=atm_ind, \
            parallel=True)
    except:
