...
for mol in mols:
    ...
    if ext:
        ...
        try:
            ...
            shell=True)
            subprocess.call(
            'scp -r {}{m}/05-MD/amber14sb_tip4pd.ff {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/'.format(srcdir,svr,m=mol),
            shell=True)
            subprocess.call(
            'scp {}{m}/05-MD/min.tpr {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/'.format(srcdir,svr,m=mol),
            shell=True)
        except:
            print( 'Unable to transfer top/gro files')
            # edit working directory in .sh run file
