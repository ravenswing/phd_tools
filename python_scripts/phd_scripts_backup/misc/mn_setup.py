...
for mol in mols:
    ...
    if ext:
        try:
            subprocess.call(
                "scp {}{m}/mdrun.tpr {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/".format(
                    srcdir, svr, m=mol
                ),
                shell=True,
            )
            ...
            subprocess.call(
                "scp {}{m}/{}.cpt {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/".format(
                    srcdir, stem, svr, m=mol
                ),
                shell=True,
            )
        except:
        ...
        data[13] = (
            "mpirun $GMX  mdrun -v -s MDrun_ex.tpr -cpi {}.cpt -deffnm {} -append -maxh 23.5 \n".format(
                stem, nxt
            )
        )
        ...
        try:
            subprocess.call(
                "scp {}{m}/05-MD/{m}_1barframe.gro {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/".format(
                    srcdir, svr, m=mol
                ),
                shell=True,
            )
            subprocess.call(
                "scp {}{m}/05-MD/{m}.top {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/".format(
                    srcdir, svr, m=mol
                ),
                shell=True,
            )
            subprocess.call(
                "scp -r {}{m}/05-MD/amber14sb_tip4pd.ff {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/".format(
                    srcdir, svr, m=mol
                ),
                shell=True,
            )
            subprocess.call(
                "scp {}{m}/05-MD/min.tpr {}:/gpfs/scratch/pr1eeq00/pr1eeq09/{m}/".format(
                    srcdir, svr, m=mol
                ),
                shell=True,
            )
        ...
        for fl in ["mdrun.sh", "mdcont.sh"]:
            with open("Scripts/MN/{}".format(fl), "r") as file:
                data = file.readlines()
            #  print data
            print("Editing line:  {}".format(data[6]))
            data[1] = '#SBATCH --job-name="MD_{}" \n'.format(mol)
            with open("Scripts/MN/{}".format(fl), "w") as file:
                file.writelines(data)
...
print(
    "----------  All {} molecules successfully prepared  ----------".format(len(mols))
)
