...
parser.add_argument("-ebetac", help="use precalculated ebetac list, if omitted use FES files")
parser.add_argument("-savelist", help="save ebetac list into this file")
parser.add_argument("-rb", "--rbias", action='store_true', help="rbias included, no need to use fes files or ebetac list")
...
parser.add_argument("-stride", type=int, default=0, help="number of rows to load from a colvar in one stride, outputs multiple files")

parser.add_argument("-bin", type=int, nargs='+', default=[ 50 ], help="number of bins for the reweighted FES (default: %(default)s for each CV)")
...
if len(ngrid) != numrewt:
    if len(ngrid) == 1:
        ...
        print ("ERROR: the number of grid dimensions (%d) does not match the number of reweighting CVs (%d)" % (len(ngrid), numrewt))
        exit(1)
    ...
    if ebetacFile:
        ...
        for i in range(numdat):
            ...
            if verbose and numdat > 10 and i % (numdat//10) == 0: 
                print ("%d of %d (%.0f%%) done" % (i,numdat,(i*100./numdat)))
...
for row in colvar:
    ...
    if stride > 0 and i % stride == 0:
        ...
        filecount += 1
        if verbose: print ("Saving partial results on %s" % out_fes_name)
