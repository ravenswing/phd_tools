...
PARSER.add_argument(
    ...
    type=str,
    default="note",
    help="Generic note (default: %(default)s)",
    required=False,
)
PARSER.add_argument(
    "-dir",
...
PARSER.add_argument(
    ...
    type=str,
    default=None,
    help="new Meeting (default: %(default)s)",
    required=False,
)
PARSER.add_argument(
    "-log", action="store_true", help="Analyse SWISH data (default: %(default)s)"
)
...
def log():
    ...
    if not path.exists():
        with path.open(mode="w+") as f:
            f.write(
                "\n\n# {} {} LOG FILE\n\n".format(
                    today.strftime("%B").upper(), today.strftime("%Y")
                )
            )
    new_lines = quicknote()
