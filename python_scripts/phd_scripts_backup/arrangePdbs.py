...
parser = argparse.ArgumentParser(
    ...
    description="Hydrophobic Seq. Builder",
    epilog=" ",
)
...
parser.add_argument(
    ...
    type=int,
    default=12,
    help="Number of residues in each peptide " + "(default: %(default)s)",
)
parser.add_argument(
    "-row",
    type=int,
...
for i in np.arange(np.prod(size))[1:]:
    alter_line = "cmd.alter(\"{}_{}\" , 'resi=str(int(resi)+{l})')\n".format(
        stem, i, l=(args.len + 1) * i if "_Ncapped" in args.pdb else (args.len * i)
    )
    with open("temp.py", "a") as f:
        f.write(alter_line)
...
with open("temp.py", "a") as f:
    f.write(
        "cmd.extract('tosave', 'all')\ncmd.save('{o}/{o}.pdb', 'tosave')".format(
            o=outname
        )
    )
...
ri = [1, 10, 11, 19]
ri_command = "ri {n[0]} | ri {n[1]}-{n[2]} | ri {n[3]}".format(
    n=[x + 1 for x in ri] if "_Ncapped" in args.pdb else ri
)
...
box_command = (
    ...
    "-bt triclinic -box {} {} {} -n i.ndx".format(dim_xy[0], dim_xy[1], dim_z)
)
...
line = line.replace("NAME", "export name={}".format(outname))
line = line.replace("NUMPEP", str(np.prod(size)))
line = line.replace("NUMCOMMAND", num_command)
line = line.replace("TERCOMMAND", ter_command)
line = line.replace("RICOMMAND", ri_command)
line = line.replace("BOXCOMMAND", box_command)
line = line.replace("NDXCOMMAND", index_command)
