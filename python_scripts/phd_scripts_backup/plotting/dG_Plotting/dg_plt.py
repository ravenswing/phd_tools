...
def swish_scatter(csv, swish=True):
    ...
    ax.scatter(
        x="exp", y="swish", data=df, marker="D", s=6, c="k", label="SWISH", zorder=2
    )
    ax.plot(x, x, "--k")
    ax.plot(x, x + 2, "--g")
    ax.plot(x, x - 2, "--g")
    ...
    for i, j, n in zip(df["exp"], df["swish"], df["pdb"]):
        ...
        # arrowprops=dict(arrowstyle="wedge,tail_width=0.5", alpha=0.1))
    s = "9-8" if swish else "ALL"
    fig.savefig("SWISH_dG_{}.png".format(s), dpi=300, transparent=True)
    ...
    for site in [0, 1, 2]:
        ...
        for fs in [1, 2]:
            ...
            for i, j, n in zip(to_plot["exp"], to_plot["fs" + str(fs)], to_plot["pdb"]):
                ...
                # arrowprops=dict(arrowstyle="wedge,tail_width=0.5", alpha=0.1))
            fig.text(
                0.31 + ((fs - 1) * 0.41), 0.9, "FS" + str(fs), ha="center", fontsize=14
            )

    s = "9-8" if swish else "ALL"
    fig.savefig("Split_dG_{}.png".format(s), dpi=300, transparent=True)
