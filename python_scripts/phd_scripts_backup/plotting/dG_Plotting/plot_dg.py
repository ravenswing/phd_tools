...
comet_codes = ["5alg", "5am3", "5aly", "5alp", "5aia", "5alt", "5akk", "5akg", "5alx"]
...
def ddg_scatter(csv):
    ...
    plt.ylabel(
        "Deviation from $\mathrm{\Delta G_{exp}}$ /" + " kcal $\mathrm{mol^{-1}}$"
    )
    plt.grid(alpha=0.5)
    plt.legend()
    plt.savefig(csv.split(".")[0] + "_scatter.png", dpi=300, transparent=True)
...
def dg_scatter(ax, df, Y):
    ...
    # plot data
    ax.scatter(x="exp", y=Y, data=df, marker="D", s=8, c="k", zorder=2)
...
def split_plot(csv):
    ...
    for site in [0, 1, 2]:
        ...
        for fs in [1, 2]:
            ...
            fig.text(
                0.31 + ((fs - 1) * 0.41), 0.9, labels[fs - 1], ha="center", fontsize=18
            )
            fig.text(
                0.31 + ((fs - 1) * 0.41),
                0.05,
...
def ind_scatterplot(csv, method):
    ...
    axes.set_xlabel("$\mathrm{\Delta G_{exp}}$ / kcal $\mathrm{mol^{-1}}$", fontsize=11)

    axes.set_ylabel(
        "$\mathrm{\Delta G_{calc}}$ / kcal $\mathrm{mol^{-1}}$", fontsize=11
    )

    fig.savefig(
        "./Ind_dG_{}.png".format(method), dpi=300, transparent=True, bbox_inches="tight"
    )
...
def swish_diftimes(timestamps, exclusions=None):
    ...
    for t in timestamps:
        ...
        dg_data = (
            df.copy(deep=True)
            if t == 300
            else pd.read_csv("SWISH_dG_{}ns.csv".format(t), sep=",")
        )
        # replace data values
        if t > 301:
            for p in dg_data.pdb.values:
                ...
                # print(dg_data)
                df.loc[df["pdb"] == p, ["swish"]] = dg_data.loc[
                    dg_data["pdb"] == p, ["r1"]
                ].values[0][0]
        # filter out exclusions
        df_ex = df[~df["pdb"].isin(exclusions[t])] if exclusions is not None else df
    ...
    fig.savefig(
        "SWISH_Times_dG{}.png".format(s), dpi=300, transparent=True, bbox_inches="tight"
    )
...
def write_stats(csv):
    ...
    with path.open(mode="w+") as f:
        ...
        for site, group in grouped:
            ...
            f.write(
                "|{}| RHS |{}| {} |\n".format(
                    site, " | ".join(["{:8.6f}".format(n) for n in stat_list1]), r
                )
            )
            fs2_val = group.fs2
            ...
            f.write(
                "| {} | LHS | {} | {} |\n".format(
                    site, " | ".join(["{:8.6f}".format(n) for n in stat_list2]), r
                )
            )
    ...
    with path.open(mode="a") as f:
        ...
        for m in ["fs1", "fs2", "swish", "comet"]:
            ...
            f.write(
                "| {} | {} | {} |\n".format(
                    m, " | ".join(["{:8.6f}".format(n) for n in stat_list]), r
                )
            )
...
def logP(csv):
    ...
    ax.axhline(y=0.0, xmin=0.0, xmax=1, c="k", ls="--", lw=1)
    ax.set_ylabel("$\mathrm{H_{diff}}$")
    ax.set_xlabel("log P")
    fig.text(
        0.3, 0.8, "$\mathrm{{ R^2 }}$ = {:3.2f}".format(R), ha="center", fontsize=10
    )
    fig.savefig("logP.png", dpi=300, transparent=True, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.axhline(y=0.0, xmin=0.0, xmax=1, c="k", ls="--", lw=1, alpha=0.8)
    ax = sns.kdeplot(
        low_clust["logP"], low_clust["m"], cmap="Reds", shade=True, shade_lowest=False
    )
    ax = sns.kdeplot(
        high_clust["logP"],
        high_clust["m"],
    ...
    ax.set_ylabel("$\mathrm{H_{diff}}$")
    f.savefig("logP_v2.png", dpi=300, transparent=True, bbox_inches="tight")
