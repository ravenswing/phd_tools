...
if DOWNLOAD:
    ...
    ## make new directory and download COLVAR & HILLS files
        try:
            ...
            subprocess.call("scp {}:{}{m}/{f}/COLVAR monitor/{m}_{f}/{m}.colvar".format(remote_svr,remote_pth,f=folder,m=mol), shell=True)
            subprocess.call("scp {}:{}{m}/{f}/HILLS monitor/{m}_{f}/{m}.hills".format(remote_svr,remote_pth,f=folder,m=mol), shell=True)
        except:
...
########################################################
    ...
    ## organise data into plotable arrays
        ...
        plt.figure()
        plt.contourf(x,y, FES[mol][2]/4.184 ,conts,cmap='CMRmap', )
