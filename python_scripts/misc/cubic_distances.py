...
# Load in oxygen data
oxy_list = pd.read_csv('{}/atom_refs/cubic_oxy.txt'.format(srcdir), names=colnames, delimiter='\t')
...
nit_list = pd.read_csv('{}/atom_refs/cubic_nit.txt'.format(srcdir), names=colnames, delimiter='\t')
...
for r in range(len(iad)): 
    ...
    for i in range(len(segments)):
        ...
        traj = md.load_xtc('{}/{m}/{t}'.format(srcdir, m=mol, t=target),\
        ...
        np.save("{}/{m}/{m}_{l}-{u}_interactions.npy".format(srcdir, m=mol, l=lower, u=upper), interactions)
